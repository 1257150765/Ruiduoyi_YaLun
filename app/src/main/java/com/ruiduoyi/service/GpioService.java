...
import com.example.sensor.GPIOUtil;
...
public class GpioService extends Service implements GPIOUtil.GpioChangeListener {
    private static final String TAG = GpioService.class.getSimpleName();
    private int i=0;
    ...
    public void initGpio(){
        GPIOUtil.getInstance().setListener(this);

        //detect.Humansensor_Settime(value, fd);	//感应设置时间接口：value为写入时间，单位为秒
    ...
    @Override
    public void onChange(final int index, boolean level) {
        long time=System.currentTimeMillis();
        Date date=new Date(time);
        SimpleDateFormat format2=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
        ...
        if(level){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    dataBase.insertGpio(mac,jtbh,"A",""+index,ymd_hms,1,"");
                    dataBase.insertCollGpio(mac,jtbh,"A",""+index,ymd_hms,1,"");
                    //dataBase.insertGpio2(mac,jtbh,"A","1",ymd_hms,1,"");
                    dataBase.selectGpio();
                }
            }).start();
